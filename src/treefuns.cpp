...
//--------------------------------------------------
//fit tree at matrix of x, matrix is stacked columns x[i,j] is *(x+p*i+j) (return fv is specified in input arguments)
void fit(tree& t, xinfo& xi, size_t p, size_t n, double *x,  double* fv)
{
  tree::tree_p bn;
  for(size_t i=0;i<n;i++) {
    //CL: x+i*p is the starting location of the i-th row of x
    bn = t.bn(x+i*p,xi);
    fv[i] = bn->gettheta();
  }
}
